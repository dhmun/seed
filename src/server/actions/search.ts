...
import spotifyClient, { type SpotifyTrack, type SpotifyArtist } from '@/lib/spotify-client';
  ...
  try {
    ...
    return tracks
      ...
      .map((track: SpotifyTrack): Content => ({
        ...
        backdrop_url: track.album.images[0]?.url || undefined,
        size_mb: Math.floor(Math.random() * (200 - 50 + 1)) + 50, // 50MB ~ 200MB
        is_active: true,
        tmdb_id: undefined,
        tmdb_type: undefined,
        release_date: track.album.release_date || undefined,
        genre_ids: [],
  ...
  try {
    ...
    return artists
      ...
      .map((artist: SpotifyArtist): Content => ({
        ...
        backdrop_url: artist.images[0]?.url || undefined,
...
// 유틸리티 함수들
function detectLanguage(trackName: string, artistName: string): string {
  // 한글 문자 포함 여부 체크
